---
title: "Medicinal Clocksmith"
type: "background"
aon_id: "background-608"
aon_url: "https://2e.aonprd.com/Backgrounds.aspx?ID=608"
tags:
  - pf2e/background
  - rarity/common
attribute:
  - "Intelligence"
  - "Wisdom"
feat:
  - "Risky Surgery"
is_general_background: true
is_standard_ancestry_feat: false
pfs: "Standard"
primary_source: "Guns & Gears"
primary_source_category: "Rulebooks"
rarity: "common"
release_date: "2021-10-13"
skill:
  - "Medicine"
  - "Engineering Lore"
source:
  - "Guns & Gears"
source_category:
  - "Rulebooks"
---

# Medicinal Clocksmith  *Background*

**Source** [[Sources/Guns & Gears]] pg. 45

While others might have looked to clockwork as a mechanical innovation, you see the potential in the technology to improve the health of patients. You might be an experienced field medic who reached for a scrapped construct to help a wounded soldier, or a vanguard surgeon who’s found replacing organs with clockwork more effective than repairing them. Perhaps you’ve also worked to “optimize” organic bodies with fine machinery, with or without the permission of your patients.

Choose two attribute boosts. One must be to Intelligence
or Wisdom, and one is a free attribute boost.

You're trained in the [[Rules/Skills/Medicine]] skill, and the [Engineering Lore](https://2e.aonprd.com/skills/lore) skill. You gain the [[Feats/Risky Surgery]] skill feat.

---

Source on Archives of Nethys: [Medicinal Clocksmith](https://2e.aonprd.com/Backgrounds.aspx?ID=608)
