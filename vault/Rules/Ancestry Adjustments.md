---
title: "Ancestry Adjustments"
type: "rules"
aon_id: "rules-3369"
aon_url: "https://2e.aonprd.com/Rules.aspx?ID=3369"
tags:
  - pf2e/rules
  - rarity/common
breadcrumbs:
  - "Chapter 1: NPC Gallery"
is_standard_ancestry_feat: false
next_link:
  - "label Chapter 2: Ancestry NPCs"
  - "url /Rules.aspx?ID=3367"
previous_link:
  - "label Troops"
  - "url /Rules.aspx?ID=3364"
primary_source: "NPC Core"
primary_source_category: "Rulebooks"
rarity: "common"
release_date: "2025-02-26"
source:
  - "NPC Core"
source_category:
  - "Rulebooks"
---

# Ancestry Adjustments  *Rules*

**Source** [[Sources/NPC Core]] pg. 167

Many kinds of people call Golarion home. While most are humans, many are not. For important NPCs, it is generally more effective to use the full creature creation rules if you have time. Sometimes, though, you need an NPC with a different ancestry in a hurry. Like the elite and weak adjustments detailed in Monster Core, the special adjustments found [here](MonsterTemplates.aspx) can be used to quickly customize any human into one of many different ancestries.

 As with any adjustments, these are meant to be fast, not comprehensive. It's best to check whether the adjustment makes the creature too powerful or contradicts other cultural precedents set in place. For example, when adding the [[Bestiary/Adjustments/Lizardfolk|lizardfolk adjustment]] to iruxi adapted to a desert environment, adding a swim Speed may not make sense.

 These adjustments don't include ancestral weaponry, Hit Points, or attribute preferences. Additional adjustments to account for these should refer to the full character creation rules to ensure changes stay within level expectations.

---

Source on Archives of Nethys: [Ancestry Adjustments](https://2e.aonprd.com/Rules.aspx?ID=3369)
