---
title: "Animate Object"
type: "ritual"
aon_id: "ritual-107"
aon_url: "https://2e.aonprd.com/Rituals.aspx?ID=107"
tags:
  - pf2e/ritual
  - rarity/uncommon
  - trait/uncommon
actions: "1 day"
actions_number: 86400
cost: "rare oils, see Creature Creation Rituals"
heighten_group:
  - "2nd rank"
heighten_level:
  - 2
is_standard_ancestry_feat: false
level: 2
pfs: "Standard"
primary_check: "Arcana (expert)"
primary_source: "Player Core"
primary_source_category: "Rulebooks"
range: 10
rarity: "uncommon"
release_date: "2023-11-15"
secondary_casters: 1
secondary_check: "Crafting"
source:
  - "Player Core"
source_category:
  - "Rulebooks"
target: "1 object"
trait:
  - "Uncommon"
trait_group:
  - "Rarity"
---

# Animate Object  *Ritual 2*

**Traits** [[Traits/Uncommon (Player Core)|Uncommon]]

**Source** [[Sources/Player Core]] pg. 390

**Cast** 1 day

**Cost** rare oils, see Creature Creation Rituals

**Secondary Casters** 1

**Primary Check** [[Rules/Skills/Arcana]] (expert)

**Secondary Checks**  Crafting

**Range** 10 feet

**Target(s)** 1 object

---

You transform the target into an animated object with a level up to that allowed by the Creature Creation Rituals table and of a type corresponding to the object (so a broom would become an [[Bestiary/Animated Broom|animated broom]]).

## Creature Creation Rituals

| **Creature Level** | **Ritual Rank** | **Required Cost** |
| --- | --- | --- |
| –1 or 0 | 2 | 15 gp |
| 1 | 2 | 60 gp |
| 2 | 3 | 105 gp |
| 3 | 3 | 180 gp |
| 4 | 4 | 300 gp |
| 5 | 4 | 480 gp |
| 6 | 5 | 750 gp |
| 7 | 5 | 1,080 gp |
| 8 | 6 | 1,500 gp |
| 9 | 6 | 2,100 gp |
| 10 | 7 | 3,000 gp |
| 11 | 7 | 4,200 gp |
| 12 | 8 | 6,000 gp |
| 13 | 8 | 9,000 gp |
| 14 | 9 | 13,500 gp |
| 15 | 9 | 19,500 gp |
| 16 | 10 | 30,000 gp |
| 17 | 10 | 45,000 gp |

**Critical Success** The target becomes an animated object of the appropriate type. If it's at least 4 levels lower than you, you can make it a minion. This gives it the [[Traits/Minion|minion]] trait, meaning it can use 2 actions when you command it, and commanding it is a single action that has the [[Traits/Auditory|auditory]] and [[Traits/Concentrate|concentrate]] traits. You can have a maximum of four minions under your control. If it doesn't become a minion, you can give it one simple command. It pursues that goal single-mindedly, ignoring any of your subsequent commands.  

**Success** As critical success, except an animated object that doesn't become your minion stays in place and attacks anyone that attacks it or tries to steal or move it, rather than following your command.  

**Failure** You fail to create the animated object.  

**Critical Failure** You create the animated object, but it goes berserk and attempts to destroy you.

---

Source on Archives of Nethys: [Animate Object](https://2e.aonprd.com/Rituals.aspx?ID=107)
