---
title: "Bottomless Stomach"
type: "spell"
aon_id: "spell-2648"
aon_url: "https://2e.aonprd.com/Spells.aspx?ID=2648"
tags:
  - pf2e/spell
  - rarity/common
  - trait/concentrate
  - trait/extradimensional
  - trait/manipulate
actions: "Three Actions"
actions_number: 6
deity:
  - "Lurlup"
  - "Orgesh"
  - "Vapula"
duration: 3600
heighten:
  - "5th"
heighten_group:
  - "3rd rank"
  - "Heightened 5th rank"
heighten_level:
  - 3
  - 5
is_standard_ancestry_feat: false
level: 3
pfs: "Standard"
primary_source: "Impossible Magic"
primary_source_category: "Rulebooks"
range: 0
rarity: "common"
release_date: "2026-07-30"
source:
  - "Impossible Magic"
source_category:
  - "Rulebooks"
spell_type: "Spell"
target: "1 willing creature"
tradition:
  - "Arcane"
  - "Occult"
  - "Primal"
trait:
  - "Concentrate"
  - "Extradimensional"
  - "Manipulate"
trait_group:
  - "Mechanics"
---

# Bottomless Stomach three actions  *Spell 3*

**Traits** [[Traits/Concentrate (Player Core)|Concentrate]], [[Traits/Extradimensional (Player Core)|Extradimensional]], [[Traits/Manipulate (Player Core)|Manipulate]]

**Source** [[Sources/Impossible Magic]] pg. 126

**Traditions**
[[Magic/Traditions/Arcane]], [[Magic/Traditions/Arcane|Occult]], [[Magic/Traditions/Arcane|Primal]]

**Deities**
[[Setting/Deities/Lurlup]], [[Setting/Deities/Orgesh]], [[Setting/Deities/Vapula]]

**Range** touch

**Target** 1 willing creature

**Duration** 1 hour

---

You create a shimmering extradimensional space accessible from the target's mouth. The space can hold objects and equipment, up to a total of 10 Bulk. This extradimensional storage doesn't hamper the target's ability to eat, drink, speak (if applicable), or otherwise act, as it only opens and closes when the target chooses.

 The target can [[Actions/Interact (Player Core)|Interact]] to swallow an object of up to 1 Bulk, which doesn't harm the object or the target. If the extradimensional space is full, the target can't add any more objects until first removing one or more stored objects. Organic matter and living creatures can't be stored in this space. The extradimensional storage is obvious to any creature who looks into the target's mouth, as the entryway shimmers slightly, though this doesn't clearly reveal the contents inside.

 The target can Interact to spit out a single object of their choice, causing the object to fall to the ground in the target's space. The target can Interact three times in a row to spit out the entire contents of the extradimensional storage; the target doesn't have to take these actions all in the same turn, but if they take any other actions in between, the target has to start over. The items eject into the target's space, possibly spilling out into adjacent spaces if there's too much to fit.

 When the spell ends or the target falls [[Rules/Conditions/Unconscious (Player Core)|unconscious]], the contents of the extradimensional storage are disgorged in a riotous spew, landing in the nearest unoccupied space, a bit wet but otherwise unharmed.

---
**Heightened (5th)** The duration increases to 8 hours.

---

Source on Archives of Nethys: [Bottomless Stomach](https://2e.aonprd.com/Spells.aspx?ID=2648)
