---
title: "Twilight Lantern"
type: "equipment"
aon_id: "equipment-1451"
aon_url: "https://2e.aonprd.com/Equipment.aspx?ID=1451"
tags:
  - pf2e/equipment
  - rarity/uncommon
  - trait/light
  - trait/magical
  - trait/transmutation
  - trait/uncommon
bulk: 0.1
is_standard_ancestry_feat: false
item_category: "Held Items"
level: 6
pfs: "Limited"
primary_source: "Pathfinder #172: Secrets of the Temple City"
primary_source_category: "Adventure Paths"
primary_source_group: "Strength of Thousands"
rarity: "uncommon"
release_date: "2021-10-13"
school: "transmutation"
source:
  - "Pathfinder #172: Secrets of the Temple City"
source_category:
  - "Adventure Paths"
source_group:
  - "Strength of Thousands"
spoilers: "Strength of Thousands"
trait:
  - "Light"
  - "Magical"
  - "Transmutation"
  - "Uncommon"
trait_group:
  - "Mechanics"
  - "School"
  - "Rarity"
usage: "held in 1 hand"
---

# Twilight Lantern  *Item 6+*

> [!warning] Spoilers
> May contain spoilers from Strength of Thousands

**Traits** [[Traits/Uncommon]], [[Traits/Light]], [[Traits/Magical]], [[Traits/Transmutation]]

**Source** [[Sources/Pathfinder -172- Secrets of the Temple City|Pathfinder #172: Secrets of the Temple City]] pg. 76

**Usage** held in 1 hand

**Bulk** L

---

This elegant hooded lantern is made from onyx-black metal. In dim light, flecks of white metal speckled within the lantern's housing resemble stars in a night sky. The lantern uses oil as a standard hooded lantern, except that any light the lantern emits is converted into magical light similar to moonlight. This magical moonlight is always dim light.

**Activate** single action envision;     **Effect** You concentrate on the lantern's light and reduce or increase its radius. The light can have a minimum radius of 5 feet and a maximum radius equal to double the light's standard radius.

**Activate** two actions envision, Interact;  **Frequency** once per day;   **Effect** You raise the lantern and unleash a beam of powerful moonlight, targeting a [[Traits/Darkness|darkness]] effect within 60 feet. The lantern attempts to [[Rules/Counteracting (Core Rulebook)|counteract]] the effect with a counteract modifier of +10.

## Twilight Lantern (Lesser)  *Item 6*

**Source** [[Sources/Pathfinder -172- Secrets of the Temple City|Pathfinder #172: Secrets of the Temple City]] pg. 76

**Price** 235 gp

**Bulk** L

## Twilight Lantern (Moderate)  *Item 10*

**Source** [[Sources/Pathfinder -172- Secrets of the Temple City|Pathfinder #172: Secrets of the Temple City]] pg. 76

**Price** 850 gp

**Bulk** L

---

The counteract modifier is +17, and the maximum radius is equal to triple the light's standard radius.

## Twilight Lantern (Greater)  *Item 14*

**Source** [[Sources/Pathfinder -172- Secrets of the Temple City|Pathfinder #172: Secrets of the Temple City]] pg. 76

**Price** 3,700 gp

**Bulk** L

---

As _moderate twilight lantern_, but the counteract modifier is +21.

## Twilight Lantern (Major)  *Item 18*

**Source** [[Sources/Pathfinder -172- Secrets of the Temple City|Pathfinder #172: Secrets of the Temple City]] pg. 76

**Price** 18,750 gp

**Bulk** L

---

The counteract modifier is +28, and the maximum radius is equal to quadruple the light's standard radius.

---

Source on Archives of Nethys: [Twilight Lantern](https://2e.aonprd.com/Equipment.aspx?ID=1451)
