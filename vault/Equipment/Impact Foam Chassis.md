---
title: "Impact Foam Chassis"
type: "equipment"
aon_id: "equipment-4219"
aon_url: "https://2e.aonprd.com/Equipment.aspx?ID=4219"
tags:
  - pf2e/equipment
  - rarity/uncommon
  - trait/consumable
  - trait/gadget
  - trait/uncommon
access: "Due to the use of technology, all gadgets are uncommon or rare, though inventors and regions with access to inventors have access to uncommon gadgets."
bulk: 0.1
is_standard_ancestry_feat: false
item_category: "Consumables"
item_subcategory: "Gadgets"
level: 1
pfs: "Standard"
primary_source: "Guns & Gears"
primary_source_category: "Rulebooks"
rarity: "uncommon"
release_date: "2021-10-13"
source:
  - "Guns & Gears"
source_category:
  - "Rulebooks"
trait:
  - "Consumable"
  - "Gadget"
  - "Uncommon"
trait_group:
  - "Equipment"
  - "Rarity"
usage: "worn over armor and clothes"
---

# Impact Foam Chassis  *Item 1+*

**Traits** [[Traits/Uncommon]], [[Traits/Consumable]], [[Traits/Gadget (Guns & Gears (Remastered))|Gadget]]

**Source** [[Sources/Guns & Gears]] pg. 70

_[**PFS Note**](https://2e.aonprd.com/pathfinder-society) All Pathfinder Society agents have access to all uncommon clockwork items and gadgets from Chapters 1 and 2 of this book_

**Usage** worn over armor and clothes

**Bulk** L

**Access**  Due to the use of technology, all gadgets are uncommon or rare, though inventors and regions with access to inventors have access to uncommon gadgets.

**Activate**

see below

---

While you’re wearing an impact foam chassis wrapped around your body, it Activates automatically whenever you [[Rules/Falling (Player Core)|fall]] at least 10 feet, causing a layer of soft, flexible impact foam to expand beneath and around you to cushion the fall. The impact foam decreases the falling damage by up to the listed amount, taking damage equal to the amount of falling damage the foam reduced. If this is enough to destroy the foam, it disperses immediately. However, if the foam has any Hit Points remaining, it remains surrounding you for up to 1 minute. While you’re surrounded in foam, you are [[Rules/Conditions/Immobilized (Player Core)|immobilized]] and have standard cover against other creatures, and other creatures have standard cover against you. The foam has an [[Actions/Escape (Player Core)|Escape]] DC of 15; a creature can also remove it by spending three [[Actions/Interact (Player Core)|Interact]] actions. You or others can also attack the foam (the foam has an AC of 12), which disperses if reduced to 0 Hit Points.

## Impact Foam Chassis (Lesser)  *Item 1*

**Source** [[Sources/Guns & Gears]] pg. 70

**Price** 3 gp

---

A lesser impact foam chassis prevents up to 20 falling damage and has 20 Hit Points.

## Impact Foam Chassis (Moderate)  *Item 4*

**Source** [[Sources/Guns & Gears]] pg. 70

**Price** 15 gp

---

A moderate impact foam chassis prevents up to 50 falling damage and has 40 Hit Points.

## Impact Foam Chassis (Greater)  *Item 9*

**Source** [[Sources/Guns & Gears]] pg. 70

**Price** 150 gp

---

A greater impact foam chassis prevents up to 100 falling damage and has 100 Hit Points.

## Impact Foam Chassis (Major)  *Item 14*

**Source** [[Sources/Guns & Gears]] pg. 70

**Price** 650 gp

---

A major impact foam chassis prevents up to 200 falling damage and has 200 Hit Points.

---

Source on Archives of Nethys: [Impact Foam Chassis](https://2e.aonprd.com/Equipment.aspx?ID=4219)
