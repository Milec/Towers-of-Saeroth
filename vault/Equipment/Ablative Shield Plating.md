---
title: "Ablative Shield Plating"
type: "equipment"
aon_id: "equipment-4207"
aon_url: "https://2e.aonprd.com/Equipment.aspx?ID=4207"
tags:
  - pf2e/equipment
  - rarity/uncommon
  - trait/consumable
  - trait/gadget
  - trait/uncommon
access: "Due to the use of technology, all gadgets are uncommon or rare, though inventors and regions with access to inventors have access to uncommon gadgets."
bulk: 1
is_standard_ancestry_feat: false
item_category: "Consumables"
item_subcategory: "Gadgets"
level: 1
pfs: "Standard"
primary_source: "Guns & Gears"
primary_source_category: "Rulebooks"
rarity: "uncommon"
release_date: "2021-10-13"
source:
  - "Guns & Gears"
source_category:
  - "Rulebooks"
trait:
  - "Consumable"
  - "Gadget"
  - "Uncommon"
trait_group:
  - "Equipment"
  - "Rarity"
usage: "attached to a shield"
---

# Ablative Shield Plating  *Item 1+*

**Traits** [[Traits/Uncommon]], [[Traits/Consumable]], [[Traits/Gadget (Guns & Gears (Remastered))|Gadget]]

**Source** [[Sources/Guns & Gears]] pg. 67

_[**PFS Note**](https://2e.aonprd.com/pathfinder-society) All Pathfinder Society agents have access to all uncommon clockwork items and gadgets from Chapters 1 and 2 of this book_

**Usage** attached to a shield

**Bulk** 1

**Access**  Due to the use of technology, all gadgets are uncommon or rare, though inventors and regions with access to inventors have access to uncommon gadgets.

---

Ablative shield plating is an amalgam device that provides a layer of additional protection for a shield. Ablative shield plating is cobbled together from parts of other shields, wires, scraps, and bits of detritus, adding mass to a shield. The process of attaching ablative shield plating takes 10 minutes, and you can’t attach ablative shield plating to a shield that has any other attached item, such as a [[Equipment/Weapons/Shield Boss (Player Core)|shield boss]] or [[Equipment/Weapons/Shield Spikes (Player Core)|shield spikes]]. When the shield is damaged, this additional material crumples, breaks, and falls apart, absorbing some of the energy of the blow. Otherwise, the additional material slowly crumbles over time. Applying the plating grants the shield [[Rules/Temporary Hit Points (Player Core)|temporary Hit Points]] that last for the stated duration or until lost. Removing the plating early destroys it.

## Ablative Shield Plating (Lesser)  *Item 1*

**Source** [[Sources/Guns & Gears]] pg. 67

**Price** 4 gp

---

The shield gains 5 temporary Hit Points that last for 1 minute or until lost.

## Ablative Shield Plating (Moderate)  *Item 3*

**Source** [[Sources/Guns & Gears]] pg. 67

**Price** 12 gp

---

The shield gains 10 temporary Hit Points that last for 10 minutes or until lost.

## Ablative Shield Plating (Greater)  *Item 7*

**Source** [[Sources/Guns & Gears]] pg. 67

**Price** 70 gp

---

The shield gains 20 temporary Hit Points that last for 1 hour or until lost.

## Ablative Shield Plating (Major)  *Item 13*

**Source** [[Sources/Guns & Gears]] pg. 67

**Price** 600 gp

---

The shield gains 30 temporary Hit Points that last for 1 hour or until lost.

## Ablative Shield Plating (True)  *Item 19*

**Source** [[Sources/Guns & Gears]] pg. 67

**Price** 8,000 gp

---

The shield gains 40 temporary Hit Points that last for 1 hour or until lost.

---

Source on Archives of Nethys: [Ablative Shield Plating](https://2e.aonprd.com/Equipment.aspx?ID=4207)
