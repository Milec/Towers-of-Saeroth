---
title: "Injigo's Loving Embrace"
type: "equipment"
aon_id: "equipment-3147"
aon_url: "https://2e.aonprd.com/Equipment.aspx?ID=3147"
tags:
  - pf2e/equipment
  - rarity/unique
  - trait/magical
  - trait/transmutation
  - trait/unique
bulk: 1
is_standard_ancestry_feat: false
item_category: "Held Items"
level: 9
price: 67500
primary_source: "Pathfinder #198: No Breath to Cry"
primary_source_category: "Adventure Paths"
primary_source_group: "Season of Ghosts"
rarity: "unique"
release_date: "2023-12-25"
school: "transmutation"
skill:
  - "Athletics"
source:
  - "Pathfinder #198: No Breath to Cry"
source_category:
  - "Adventure Paths"
source_group:
  - "Season of Ghosts"
spoilers: "Season of Ghosts"
trait:
  - "Magical"
  - "Transmutation"
  - "Unique"
trait_group:
  - "Mechanics"
  - "School"
  - "Rarity"
usage: "held in 2 hands"
---

# Injigo's Loving Embrace  *Item 9*

> [!warning] Spoilers
> May contain spoilers from Season of Ghosts

**Traits** [[Traits/Unique]], [[Traits/Magical]], [[Traits/Transmutation]]

**Source** [[Sources/Pathfinder -198- No Breath to Cry|Pathfinder #198: No Breath to Cry]] pg. 77

**Price** 675 gp

**Usage** held in 2 hands

**Bulk** 1

---

This net was woven by the jorogumo Injigo, who presented it as a gift to her gnome sweetheart, Mara, to keep her safe in their mountain cottage when Injigo left on hunting expeditions.

 _Injigo's Loving Embrace_ functions as a typical [[Equipment/Net (Player Core)|net]]. You gain a +1 item bonus to Athletics checks made to [[Actions/Grapple (Player Core)|Grapple]] with the net.

**Activate** two actions envision, Interact;  **Frequency** once per day;   **Effect** You spin the net in the direction of an incorporeal creature within 30 feet that you can see. Lines of silken force whip out to lash around the incorporeal creature, who must attempt a DC 25 Reflex save to avoid becoming [[Rules/Conditions/Immobilized (Player Core)|immobilized]] until the end of your next turn. You can Sustain this activation for up to 1 minute, and the creature must succeed at a DC 25 check to [[Actions/Escape (Player Core)|Escape]] the net.

**Activate**  10 minutes (envision, Interact);  **Frequency** once per week;   **Effect** You wrap the net around you like a blanket as you prepare to sleep. As long as you get 8 hours of sleep or analogous rest, you dream of writing love letters to someone you adore: this could be an actual paramour, someone you're interested in, or someone unknown whose identity you don't recall after waking. When you wake, you're filled with a sense of melancholia about those unsent letters, but these feelings distract you from other, more unpleasant thoughts, granting you resistance 5 to mental damage and a +1 item bonus to saving throws against mental effects for the next 8 hours.

---

Source on Archives of Nethys: [Injigo's Loving Embrace](https://2e.aonprd.com/Equipment.aspx?ID=3147)
