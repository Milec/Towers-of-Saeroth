---
title: "Wayfinder"
type: "equipment"
aon_id: "equipment-3117-3065"
aon_url: "https://2e.aonprd.com/Equipment.aspx?ID=3117"
tags:
  - pf2e/equipment
  - rarity/uncommon
  - trait/invested
  - trait/magical
  - trait/uncommon
access: "member of the Pathfinder Society"
bulk: 0
is_standard_ancestry_feat: false
item_category: "Worn Items"
item_subcategory: "Other Worn Items"
level: 2
pfs: "Standard"
price: 2800
primary_source: "GM Core"
primary_source_category: "Rulebooks"
rarity: "uncommon"
release_date: "2023-11-15"
source:
  - "GM Core"
source_category:
  - "Rulebooks"
trait:
  - "Invested"
  - "Magical"
  - "Uncommon"
trait_group:
  - "Equipment"
  - "Mechanics"
  - "Rarity"
usage: "worn"
---

# Wayfinder  *Item 2+*

**Traits** [[Traits/Uncommon (Player Core)|Uncommon]], [[Traits/Invested (GM Core)|Invested]], [[Traits/Magical (Player Core)|Magical]]

**Source** [[Sources/GM Core]] pg. 297

**Usage** worn

---

This compact compass repurposes ancient technology to draw fantastic powers from the mysterious magical items called [[Equipment/Aeon Stone|aeon stones]]. It serves as a badge of office for agents of the Pathfinder Society and as a status symbol among adventurers of any stripe.

## Wayfinder  *Item 2*

**Source** [[Sources/GM Core]] pg. 297

**Price** 28 gp

**Usage** worn

**Access** member of the Pathfinder Society

---

This compact compass repurposes ancient technology to draw fantastic powers from the mysterious magical items called [[Equipment/Aeon Stone|aeon stones]]. It serves as a badge of office for agents of the Pathfinder Society and as a status symbol among adventurers of any stripe. A wayfinder functions as a compass.  
 An indentation in the middle of the wayfinder can hold a single _aeon stone_. Placing an aeon stone in this indentation provides you all the benefits of having the aeon stone orbiting your head, but it protects the stone from being noticed or stolen as easily. You invest a wayfinder and the aeon stone within it simultaneously, and they count as only one item toward your investiture limit. An invested _aeon stone_ slotted in a wayfinder also grants its resonant power.  
 If you have more than one wayfinder with an invested _aeon stone_ on your person at a given time, destructive interference from their resonance prevents you from gaining benefits from any of them. You can still benefit from additional aeon stones orbiting your head, just not in wayfinders.

**Activate—Light** single action ([[Traits/Concentrate|concentrate]])    **Effect** The _wayfinder_ is targeted by a 1st-rank [[Spells/Light|_light_]] spell.

**Craft Requirements**

---

Source on Archives of Nethys: [Wayfinder](https://2e.aonprd.com/Equipment.aspx?ID=3117)
