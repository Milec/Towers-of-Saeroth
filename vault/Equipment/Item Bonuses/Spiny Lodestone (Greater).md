---
title: "Spiny Lodestone (Greater)"
type: "item-bonus"
aon_id: "equipment-2620-2372-bonus-791"
aon_url: "https://2e.aonprd.com/Equipment.aspx?ID=2620"
tags:
  - pf2e/item-bonus
  - rarity/common
  - trait/magical
  - trait/metal
  - trait/spellheart
bulk: 0
element:
  - "Metal"
is_standard_ancestry_feat: false
item_bonus_consumable: false
item_bonus_value: 1
item_category: "Spellhearts"
level: 8
pfs: "Standard"
price: 42500
primary_source: "Rage of Elements"
primary_source_category: "Rulebooks"
rarity: "common"
release_date: "2023-08-02"
skill:
  - "Athletics"
source:
  - "Rage of Elements"
source_category:
  - "Rulebooks"
trait:
  - "Magical"
  - "Metal"
  - "Spellheart"
trait_group:
  - "Mechanics"
  - "Elemental"
  - "Equipment"
usage: "affixed to a metal armor or weapon"
---

# Spiny Lodestone  *Item 4+*

**Traits** [[Traits/Magical]], [[Traits/Metal]], [[Traits/Spellheart]]

**Source** [[Sources/Rage of Elements]] pg. 147

**Usage** affixed to a metal armor or weapon

---

This perfectly octahedral magnetite crystal is covered in a hair-like layer of iron sand that always finds its way back to shape if wiped away. The spell attack modifier of any spell cast by Activating this item is +8, and the spell DC is 18.

- **Armor** You gain a +1 item bonus to [[Rules/Sidebars/Falling|Athletics]] checks and resistance 2 to [[Traits/Nonlethal|nonlethal]] damage.
- **Weapon** After you cast a spell by Activating the _spiny lodestone_, your Strikes with the weapon deal an additional 1d4 piercing damage until the end of your next turn.

**Activate**  Cast a Spell    **Effect** You cast [[Spells/Needle Darts|_needle darts_]].

## Spiny Lodestone  *Item 4*

**Source** [[Sources/Rage of Elements]] pg. 147

**Price** 90 gp

## Spiny Lodestone (Greater)  *Item 8*

**Source** [[Sources/Rage of Elements]] pg. 147

**Price** 425 gp

---

Resistance when affixed to armor is 5, additional damage when affixed to a weapon is 1d6, the spell attack modifier is +14, and the spell DC is 24.

**Activate**  Cast a Spell **Frequency** once per day;   **Effect** You cast [[Spells/Magnetic Acceleration|_magnetic acceleration_]].

## Spiny Lodestone (Major)  *Item 12*

**Source** [[Sources/Rage of Elements]] pg. 147

**Price** 1,750 gp

---

Bonus to Athletics checks when affixed to armor is +2, resistance when affixed to armor is 10, additional damage when affixed to a weapon is 1d8, the spell attack modifier is +19, and the spell DC is 29.

**Activate**  Cast a Spell **Frequency** once per day;   **Effect** You cast 4th-rank _magnetic acceleration_.

**Activate**  Cast a Spell **Frequency** once per day;   **Effect** You cast [[Spells/Rust Cloud|_rust cloud_]].

---

Source on Archives of Nethys: [Spiny Lodestone (Greater)](https://2e.aonprd.com/Equipment.aspx?ID=2620)
