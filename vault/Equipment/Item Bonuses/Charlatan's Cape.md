---
title: "Charlatan's Cape"
type: "item-bonus"
aon_id: "equipment-3063-bonus-837"
aon_url: "https://2e.aonprd.com/Equipment.aspx?ID=3063"
tags:
  - pf2e/item-bonus
  - rarity/uncommon
  - trait/invested
  - trait/magical
  - trait/uncommon
bulk: 0.1
is_standard_ancestry_feat: false
item_bonus_consumable: false
item_bonus_value: 2
item_category: "Worn Items"
item_subcategory: "Other Worn Items"
legacy_name:
  - "Cape of the Mountebank"
level: 10
pfs: "Standard"
price: 98000
primary_source: "GM Core"
primary_source_category: "Rulebooks"
rarity: "uncommon"
release_date: "2023-11-15"
skill:
  - "Deception"
source:
  - "GM Core"
source_category:
  - "Rulebooks"
trait:
  - "Invested"
  - "Magical"
  - "Uncommon"
trait_group:
  - "Equipment"
  - "Mechanics"
  - "Rarity"
usage: "worn cloak"
---

# Charlatan's Cape  *Item 10*

**Traits** [[Traits/Uncommon (Player Core)|Uncommon]], [[Traits/Invested (GM Core)|Invested]], [[Traits/Magical (Player Core)|Magical]]

**Source** [[Sources/GM Core]] pg. 286

**Price** 980 gp

**Usage**  worn cloak

**Bulk** L

---

This bright red-and-gold cape is often interlaced with glittery threads and serves as a distraction. While wearing the cape, you gain a +2 item bonus to Deception checks.

**Activate—Puff of Smoke** two actions ([[Traits/Manipulate|manipulate]]) **Frequency** once per day;   **Effect** You cast [[Spells/Translocate|_translocate_]]. The space you leave and the one you appear in are filled with puffs of smoke that make anyone within concealed until they leave the smoke or the end of your next turn, at which point the smoke dissipates. Strong winds immediately disperse the smoke.

---

Source on Archives of Nethys: [Charlatan's Cape](https://2e.aonprd.com/Equipment.aspx?ID=3063)
