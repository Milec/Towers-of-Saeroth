---
title: "Gorget of the Primal Roar"
type: "item-bonus"
aon_id: "equipment-3085-bonus-841"
aon_url: "https://2e.aonprd.com/Equipment.aspx?ID=3085"
tags:
  - pf2e/item-bonus
  - rarity/common
  - trait/invested
  - trait/magical
bulk: 0.1
is_standard_ancestry_feat: false
item_bonus_consumable: false
item_bonus_value: 2
item_category: "Worn Items"
item_subcategory: "Other Worn Items"
level: 11
pfs: "Standard"
price: 125000
primary_source: "GM Core"
primary_source_category: "Rulebooks"
rarity: "common"
release_date: "2023-11-15"
skill:
  - "Intimidation"
source:
  - "GM Core"
source_category:
  - "Rulebooks"
trait:
  - "Invested"
  - "Magical"
trait_group:
  - "Equipment"
  - "Mechanics"
usage: "worn collar"
---

# Gorget of the Primal Roar  *Item 11*

**Traits** [[Traits/Invested (GM Core)|Invested]], [[Traits/Magical (Player Core)|Magical]]

**Source** [[Sources/GM Core]] pg. 290

**Price** 1,250 gp

**Usage** worn collar

**Bulk** L

---

This engraved duskwood gorget seems to vibrate with ferocity, granting you a +2 item bonus to Intimidation checks.

**Activate—Primal Roar** single action ([[Traits/Auditory|auditory]], [[Traits/Concentrate|concentrate]], [[Traits/Emotion|emotion]], [[Spells/Fear|_fear_]], [[Traits/Mental|mental]]) **Frequency** once during the duration of each polymorph effect;  **Requirements** You're in a non-humanoid form via a polymorph effect; **Effect** You unleash a bestial roar, attempting a single Intimidation check compared to the Will DCs of all enemies within 30 feet to impose the effects below.

**Critical Success** The creature is [[Rules/Conditions/Frightened|frightened]] 2  

**Success** The creature is frightened 1  

**Failure** The creature is unaffected

---

Source on Archives of Nethys: [Gorget of the Primal Roar](https://2e.aonprd.com/Equipment.aspx?ID=3085)
