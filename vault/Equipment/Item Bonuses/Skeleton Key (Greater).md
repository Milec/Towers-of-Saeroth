---
title: "Skeleton Key (Greater)"
type: "item-bonus"
aon_id: "equipment-3030-2889-bonus-863"
aon_url: "https://2e.aonprd.com/Equipment.aspx?ID=3030"
tags:
  - pf2e/item-bonus
  - rarity/common
  - trait/magical
bulk: 0
is_standard_ancestry_feat: false
item_bonus_consumable: false
item_bonus_note: "Thievery to Pick a Lock"
item_bonus_value: 2
item_category: "Held Items"
legacy_name:
  - "Skeleton Key"
level: 11
pfs: "Standard"
price: 125000
primary_source: "GM Core"
primary_source_category: "Rulebooks"
rarity: "common"
release_date: "2023-11-15"
skill:
  - "Thievery"
source:
  - "GM Core"
source_category:
  - "Rulebooks"
trait:
  - "Magical"
trait_group:
  - "Mechanics"
usage: "held in 1 hand"
---

# Skeleton Key  *Item 5+*

**Traits** [[Traits/Magical (Player Core)|Magical]]

**Source** [[Sources/GM Core]] pg. 276

**Usage** held in 1 hand

---

A grinning skull tops the bow of this macabre key. This key can be used in place of a thieves' toolkit when attempting to [[Actions/Pick a Lock]], and it grants a +1 item bonus to the Thievery check. If the skeleton key becomes broken due to a critical failure on the check, it works as a normal thieves' toolkit and loses its benefits until repaired.

**Activate—Loosen Lock** free action ([[Traits/Manipulate|manipulate]]) **Frequency** once per day;   **Effect** The key casts [[Spells/Knock|_knock_]] on the lock you're trying to pick.

## Skeleton Key  *Item 5*

**Source** [[Sources/GM Core]] pg. 276, [[Sources/Core Rulebook]] pg. 575

**Price** 125 gp

## Skeleton Key (Greater)  *Item 11*

**Source** [[Sources/GM Core]] pg. 276, [[Sources/Core Rulebook]] pg. 575

**Price** 1,250 gp

---

The key grants a +2 item bonus, and you can activate the key once per hour.

---

Source on Archives of Nethys: [Skeleton Key (Greater)](https://2e.aonprd.com/Equipment.aspx?ID=3030)
