---
title: "Accolade Robe (Greater)"
type: "item-bonus"
aon_id: "equipment-3054-2970-bonus-903"
aon_url: "https://2e.aonprd.com/Equipment.aspx?ID=3054"
tags:
  - pf2e/item-bonus
  - rarity/common
  - trait/arcane
  - trait/focused
  - trait/invested
bulk: 0.1
is_standard_ancestry_feat: false
item_bonus_consumable: false
item_bonus_value: 3
item_category: "Worn Items"
item_subcategory: "Other Worn Items"
legacy_name:
  - "Specialist's Ring"
level: 17
pfs: "Standard"
price: 1350000
primary_source: "GM Core"
primary_source_category: "Rulebooks"
rarity: "common"
release_date: "2023-11-15"
skill:
  - "Arcana"
source:
  - "GM Core"
source_category:
  - "Rulebooks"
trait:
  - "Arcane"
  - "Focused"
  - "Invested"
trait_group:
  - "Tradition"
  - "Monster"
  - "Equipment"
usage: "worn garment"
---

# Accolade Robe  *Item 10+*

**Traits** [[Traits/Arcane (Player Core)|Arcane]], [[Traits/Focused (GM Core)|Focused]], [[Traits/Invested (GM Core)|Invested]]

**Source** [[Sources/GM Core]] pg. 284

**Usage** worn garment

**Bulk** L

---

Although not all wizards have gone through formal training, it's become tradition to enchant robes representing the arduous training required and festoon them with honors one has earned. Typically, an _accolade robe_ is styled after a single wizard school, with appropriate colors and symbols. Wearing these robes grants a +2 item bonus to Arcana checks.

 The pockets of the robe tie to an extradimensional space that can hold 1 Bulk of items, none of which can have greater than light Bulk. The items must be related to spellcasting and academics—spellbooks, scrolls, wands, scholarly journals, and other academic supplies the GM allows. These items do not count against your Bulk limit. You can Interact to retrieve or stow items normally.

**Activate—Review** single action ([[Traits/Concentrate|concentrate]], [[Traits/Manipulate|manipulate]])    **Effect** You retrieve an item of your choice from the robe's storage, then Recall Knowledge.

**Activate—Extra Credit** free action ([[Traits/Concentrate|concentrate]]) **Frequency** once per day;   **Effect** You gain 1 Focus Point, which you can spend only to cast a school spell. If you don't spend this Focus Point by the end of this turn, it is lost.

**Craft Requirements** You are a wizard of the associated school.

## Accolade Robe  *Item 10*

**Source** [[Sources/GM Core]] pg. 284

**Price** 1,000 gp

## Accolade Robe (Greater)  *Item 17*

**Source** [[Sources/GM Core]] pg. 284

**Price** 13,500 gp

---

The bonus to Arcana checks increases to +3. You can activate the robe’s Review once per minute as a free action.

---

Source on Archives of Nethys: [Accolade Robe (Greater)](https://2e.aonprd.com/Equipment.aspx?ID=3054)
