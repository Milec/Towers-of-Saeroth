---
title: "Insistent Door Knocker (Greater)"
type: "item-bonus"
aon_id: "equipment-511-596-bonus-211"
aon_url: "https://2e.aonprd.com/Equipment.aspx?ID=511"
tags:
  - pf2e/item-bonus
  - rarity/common
  - trait/conjuration
  - trait/magical
bulk: 1
is_standard_ancestry_feat: false
item_bonus_consumable: false
item_bonus_note: "Thievery checks to Pick a Lock"
item_bonus_value: 2
item_category: "Held Items"
level: 11
pfs: "Standard"
price: 125000
primary_source: "Character Guide"
primary_source_category: "Lost Omens"
rarity: "common"
release_date: "2019-10-16"
school: "conjuration"
skill:
  - "Thievery"
source:
  - "Character Guide"
source_category:
  - "Lost Omens"
trait:
  - "Conjuration"
  - "Magical"
trait_group:
  - "School"
  - "Mechanics"
usage: "held in 1 hand"
---

# Insistent Door Knocker  *Item 6+*

**Traits** [[Traits/Conjuration]], [[Traits/Magical]]

**Source** [[Sources/Character Guide]] pg. 72

**Usage** held in 1 hand

**Bulk** 1

---

This dark iron door knocker comes in a variety of shapes, such as a bird clutching a ring in its talons or a gargoyle holding a ring in its mouth. Placing the door knocker on the surface of a door (an Interact action) causes it to attach and remain in place. While in place, the door knocker whispers hints to you while you attempt to unlock the door, granting a +1 item bonus to [[Rules/Skills/Thievery]] checks to Pick a Lock. Removing the door knocker from a surface requires you to use an Interact action.

**Activate** two actions command, Interact;  **Frequency** once per day;   **Effect** You place the door knocker against a wall, floor, or ceiling. The door knocker fuses in place and creates a usable door within the surface. Any creature can move through the door normally, as if the door had always been there. The door can penetrate up to 1 foot of material, so thick material, such as heavy stone walls, can cause this effect to fail, expending its use for the day. A thin layer of metal in a wall, floor, or ceiling also causes the effect to fail. You can use an Interact action to release the door knocker from the surface and return the surface to its previous shape.

## Insistent Door Knocker  *Item 6*

**Source** [[Sources/Character Guide]] pg. 72

**Price** 225 gp

**Bulk** 1

## Insistent Door Knocker (Greater)  *Item 11*

**Source** [[Sources/Character Guide]] pg. 72

**Price** 1,250 gp

**Bulk** 1

---

The item bonus is +2, and the door knocker can penetrate up to 5 feet of material when activated.

## Insistent Door Knocker (Major)  *Item 17*

**Source** [[Sources/Character Guide]] pg. 72

**Price** 13,500 gp

**Bulk** 1

---

The item bonus is +3, and the door knocker can penetrate up to 10 feet of material when activated.

---

Source on Archives of Nethys: [Insistent Door Knocker (Greater)](https://2e.aonprd.com/Equipment.aspx?ID=511)
