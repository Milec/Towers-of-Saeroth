---
title: "Crafter's Eyepiece (Greater)"
type: "item-bonus"
aon_id: "equipment-3072-3008-bonus-881"
aon_url: "https://2e.aonprd.com/Equipment.aspx?ID=3072"
tags:
  - pf2e/item-bonus
  - rarity/common
  - trait/invested
  - trait/magical
bulk: 0
is_standard_ancestry_feat: false
item_bonus_consumable: false
item_bonus_value: 2
item_category: "Worn Items"
item_subcategory: "Other Worn Items"
legacy_name:
  - "Crafter's Eyepiece"
level: 11
pfs: "Standard"
price: 120000
primary_source: "GM Core"
primary_source_category: "Rulebooks"
rarity: "common"
release_date: "2023-11-15"
skill:
  - "Crafting"
source:
  - "GM Core"
source_category:
  - "Rulebooks"
trait:
  - "Invested"
  - "Magical"
trait_group:
  - "Equipment"
  - "Mechanics"
usage: "worn eyepiece"
---

# Crafter's Eyepiece  *Item 3+*

**Traits** [[Traits/Invested (GM Core)|Invested]], [[Traits/Magical (Player Core)|Magical]]

**Source** [[Sources/GM Core]] pg. 288

**Usage** worn eyepiece

---

This rugged metal eyepiece etched with square patterns is designed to be worn over a single eye. Twisting the lens reveals a faint three-dimensional outline of an item you plan to build or repair, with helpful labels on the component parts. You gain a +1 item bonus to Crafting checks. When you Repair an item, increase the Hit Points restored to 10 + 10 per proficiency rank on a success or 15 + 15 per proficiency rank on a critical success.

## Crafter's Eyepiece  *Item 3*

**Source** [[Sources/GM Core]] pg. 288, [[Sources/Core Rulebook]] pg. 609

**Price** 60 gp

## Crafter's Eyepiece (Greater)  *Item 11*

**Source** [[Sources/GM Core]] pg. 288, [[Sources/Core Rulebook]] pg. 609

**Price** 1,200 gp

---

The eyepiece grants a +2 item bonus and can be activated.

**Activate—Prototype**  1 minute ([[Traits/Manipulate|manipulate]]) **Frequency** once per day;   **Effect** You calibrate the eyepiece to have it cast a 5th-rank [[Spells/Creation (Player Core)|_creation_]] spell over the course of 1 minute to construct a temporary item.

---

Source on Archives of Nethys: [Crafter's Eyepiece (Greater)](https://2e.aonprd.com/Equipment.aspx?ID=3072)
