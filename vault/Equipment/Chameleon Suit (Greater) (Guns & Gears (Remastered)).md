---
title: "Chameleon Suit (Greater)"
type: "equipment"
aon_id: "equipment-1106-1063"
aon_url: "https://2e.aonprd.com/Equipment.aspx?ID=1106"
tags:
  - pf2e/equipment
  - rarity/uncommon
  - trait/clockwork
  - trait/consumable
  - trait/gadget
  - trait/uncommon
access: "Characters from Absalom, New Thassilon, Alkenstar, Dongun Hold, or especially technological regions of Arcadia like the Deadshot Lands have access to uncommon clockwork items from the first two chapters of this book. Due to the use of technology, all gadgets are uncommon or rare, though inventors and regions with access to inventors have access to uncommon gadgets."
actions: "Two Actions"
actions_number: 4
bulk: 0.1
is_standard_ancestry_feat: false
item_category: "Consumables"
item_subcategory: "Gadgets"
level: 8
pfs: "Standard"
price: 8000
primary_source: "Guns & Gears (Remastered)"
primary_source_category: "Rulebooks"
rarity: "uncommon"
release_date: "2025-02-05"
source:
  - "Guns & Gears (Remastered)"
source_category:
  - "Rulebooks"
trait:
  - "Clockwork"
  - "Consumable"
  - "Gadget"
  - "Uncommon"
trait_group:
  - "Monster"
  - "Equipment"
  - "Weapon"
  - "Rarity"
usage: "worn over armor and clothes"
---

# Chameleon Suit  *Item 4+*

**Traits** [[Traits/Uncommon]], [[Traits/Clockwork]], [[Traits/Consumable]], [[Traits/Gadget (Guns & Gears (Remastered))|Gadget]]

**Source** [[Sources/Guns & Gears (Remastered)]] pg. 67

_[**PFS Note**](https://2e.aonprd.com/pathfinder-society) All Pathfinder Society agents have access to all uncommon clockwork items and gadgets from Chapters 1 and 2 of this book_

**Usage** worn over armor and clothes

**Bulk** L

**Access** Characters from Absalom, New Thassilon, Alkenstar, Dongun Hold, or especially technological regions of Arcadia like the Deadshot Lands have access to uncommon clockwork items from the first two chapters of this book. Due to the use of technology, all gadgets are uncommon or rare, though inventors and regions with access to inventors have access to uncommon gadgets.

**Activate**

two actions

Interact

---

This complex clockwork outerwear, worn over armor or other clothing, is composed of thin, interlocking scales of myriad colors attached to numerous clockwork gears that can rotate between all visible colors and shades.

When activated, the clockwork gears work double-time to temporarily switch and adjust the scales to match the exact coloration of the area around you, allowing you to blend in as long as you stay still. For the duration, you can [[Actions/Hide]] without needing cover or concealment to do so. This doesn't allow you to [[Actions/Sneak]] without ending your movement in cover or concealment, however, as the clockwork flips over and adjusts the scales to match your background as you move, giving the impression of rippling waves of color and revealing your movement. Once the duration expires, the overworked clockwork and scales fall apart.

## Chameleon Suit  *Item 4*

**Source** [[Sources/Guns & Gears (Remastered)]] pg. 67

**Price** 16 gp

**Bulk** L

---

The effect lasts for 1 minute.

## Chameleon Suit (Greater)  *Item 8*

**Source** [[Sources/Guns & Gears (Remastered)]] pg. 67

**Price** 80 gp

**Bulk** L

---

The effect lasts for 10 minutes.

## Chameleon Suit (Major)  *Item 12*

**Source** [[Sources/Guns & Gears (Remastered)]] pg. 67

**Price** 320 gp

**Bulk** L

---

The effect lasts for 1 hour.

---

Source on Archives of Nethys: [Chameleon Suit (Greater)](https://2e.aonprd.com/Equipment.aspx?ID=1106)
