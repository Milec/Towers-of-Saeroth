---
title: "Ghost Charge (Moderate)"
type: "equipment"
aon_id: "equipment-3294-3150"
aon_url: "https://2e.aonprd.com/Equipment.aspx?ID=3294"
tags:
  - pf2e/equipment
  - rarity/common
  - trait/alchemical
  - trait/bomb
  - trait/consumable
  - trait/splash
  - trait/vitality
  - trait/positive
actions: "Single Action"
actions_number: 2
bulk: 0.1
is_standard_ancestry_feat: false
item_category: "Alchemical Items"
item_subcategory: "Alchemical Bombs"
legacy_name:
  - "Ghost Charge"
level: 3
pfs: "Standard"
price: 1000
primary_source: "Player Core 2"
primary_source_category: "Rulebooks"
rarity: "common"
release_date: "2024-08-01"
source:
  - "Player Core 2"
source_category:
  - "Rulebooks"
trait:
  - "Alchemical"
  - "Bomb"
  - "Consumable"
  - "Splash"
  - "Vitality"
  - "Positive"
trait_group:
  - "Monster"
  - "Equipment"
  - "Weapon"
  - "Mechanics"
  - "Energy"
  - "Planar"
  - "Creature Type"
usage: "held in 1 hand"
---

# Ghost Charge  *Item 1+*

**Traits** [[Traits/Alchemical (Player Core)|Alchemical]], [[Traits/Bomb (Player Core)|Bomb]], [[Traits/Consumable (Player Core)|Consumable]], [[Traits/Splash (GM Core)|Splash]], [[Traits/Vitality]]

**Source** [[Sources/Player Core 2]] pg. 285

**Usage** held in 1 hand

**Bulk** L

**Activate**

single action

Strike

---

These spring-loaded metal canisters contain a mixture of chemicals and salts that drain and disintegrate nearby undead creatures. A ghost charge deals the listed vitality damage and splash damage, though as usual for [[Traits/Vitality|vitality]] damage, this damage harms only undead and creatures with [[Bestiary/Abilities/Void Healing|void healing]]. Ghost charges are designed to explode even on contact with a spiritual substance, making them ideal for damaging [[Traits/Incorporeal (GM Core)|incorporeal]] undead. A primary target that takes damage from a ghost charge becomes [[Rules/Conditions/Enfeebled (Player Core)|enfeebled]] until the start of your next turn. Many types grant an item bonus to attack rolls.

## Ghost Charge (Lesser)  *Item 1*

**Source** [[Sources/Player Core 2]] pg. 285, [[Sources/Advanced Player's Guide]] pg. 253

**Price** 3 gp

**Bulk** L

---

The bomb deals 1d8 vitality damage and 1 vitality splash damage, and the target is enfeebled 1 until the start of your next turn.

## Ghost Charge (Moderate)  *Item 3*

**Source** [[Sources/Player Core 2]] pg. 285, [[Sources/Advanced Player's Guide]] pg. 253

**Price** 10 gp

**Bulk** L

---

You gain a +1 item bonus to attack rolls. The bomb deals 2d8 vitality damage and 2 vitality splash damage, and the target is enfeebled 1 until the start of your next turn.

## Ghost Charge (Greater)  *Item 11*

**Source** [[Sources/Player Core 2]] pg. 285, [[Sources/Advanced Player's Guide]] pg. 253

**Price** 250 gp

**Bulk** L

---

You gain a +2 item bonus to attack rolls. The bomb deals 3d8 vitality damage and 3 vitality splash damage, and the target is enfeebled 2 until the start of your next turn.

## Ghost Charge (Major)  *Item 17*

**Source** [[Sources/Player Core 2]] pg. 285, [[Sources/Advanced Player's Guide]] pg. 253

**Price** 2,500 gp

**Bulk** L

---

You gain a +3 item bonus to attack rolls. The bomb deals 4d8 vitality damage and 4 vitality splash damage, and the target is enfeebled 2 until the start of your next turn.

---

Source on Archives of Nethys: [Ghost Charge (Moderate)](https://2e.aonprd.com/Equipment.aspx?ID=3294)
