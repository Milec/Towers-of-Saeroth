---
title: "Shimmerthief"
type: "creature"
aon_id: "creature-2795"
aon_url: "https://2e.aonprd.com/Monsters.aspx?ID=2795"
tags:
  - pf2e/creature
  - rarity/uncommon
  - trait/beast
  - trait/uncommon
  - trait/medium
ac: 25
attack_bonus:
  - 17
  - 17
charisma: 0
constitution: 3
creature_ability:
  - "Echolocation"
  - "Shimmervision"
  - "Gleam Theft"
  - "Drain Light"
  - "Lasting Darkness"
dexterity: 6
fortitude_save: 12
hp: 119
immunity:
  - "blinded"
  - "dazzled"
  - "visual"
intelligence: 0
is_standard_ancestry_feat: false
language:
  - "Draconic"
level: 7
npc: false
perception: 15
primary_source: "Pathfinder #198: No Breath to Cry"
primary_source_category: "Adventure Paths"
primary_source_group: "Season of Ghosts"
rarity: "uncommon"
reflex_save: 17
release_date: "2023-12-25"
sense: "echolocation 120 feet, shimmervision"
size:
  - "Medium"
skill:
  - "Stealth"
  - "Survival"
skill_mod:
  - "stealth 17"
  - "survival 17"
source:
  - "Pathfinder #198: No Breath to Cry"
source_category:
  - "Adventure Paths"
source_group:
  - "Season of Ghosts"
speed:
  - "land 25"
spell:
  - "Darkness"
  - "Blur"
spell_attack_bonus:
  - 17
spell_dc:
  - 25
spoilers: "Season of Ghosts"
stealth: "17"
strength: 4
strike_damage_average:
  - 17
  - 19
strongest_save:
  - "ref"
  - "reflex"
tradition:
  - "Primal"
trait:
  - "Beast"
  - "Uncommon"
  - "Medium"
trait_group:
  - "Creature Type"
  - "Rarity"
weakest_save:
  - "fort"
  - "fortitude"
will_save: 15
wisdom: 4
---

# Shimmerthief

Shimmerthieves are blind, lizard-like cave dwellers with an insatiable yearning to consume the rare delicacy of light. This hunger compels them to travel beyond their subterranean lairs to the surface to feed, leaving pockets of darkness behind as they go.

 They're far from unintelligent yet prefer to avoid other sapient creatures, aware that many find them off-putting. They become independent at a young age but are protective of their kind, and they travel in small groups when able. Prudent shimmerthieves try to avoid feeding on light in busy areas, but such places are also the most tempting, as bustling populations mean more light sources.

**[[Rules/Recall Knowledge|Recall Knowledge - Beast]]**
([[Rules/Skills/Arcana]], [[Rules/Sidebars/Commanded Animals|Nature]]): DC 25

**[[Rules/Recall Knowledge|Unspecific Lore]]**: DC 23

**[[Rules/Recall Knowledge|Specific Lore]]**: DC 20

## Shimmerthief  *Creature 7*

**Traits** [[Traits/Uncommon]], Medium, [[Traits/Beast]]

**Source** [[Sources/Pathfinder -198- No Breath to Cry|Pathfinder #198: No Breath to Cry]] pg. 86

**Perception** +15; echolocation 120 feet, shimmervision

**Languages**
[[Setting/Languages/Draconic]]

**Skills**
[[Rules/Sidebars/Being Stealthy|Stealth]] +17, [[Rules/Skills/Survival]] +17

**Str** +4

**Dex** +6

**Con** +3

**Int** +0

**Wis** +4

**Cha** +0

**Echolocation** A shimmerthief can use their hearing as a precise sense at the listed range.  
**Shimmervision** A shimmerthief's sight allows them to sense sources of light from the same distance as creatures with standard vision but doesn't allow them to make out details. They can't detect anything other than light sources using this vision.  

---

**AC** 25

**Fort** +12

**Ref** +17

**Will** +15

**HP** 119

**Immunities**
blinded, dazzled, visual

**Gleam Theft** ([[Traits/Aura (Player Core)|aura]], [[Traits/Darkness (Player Core)|darkness]], [[Traits/Primal (Player Core)|primal]]) 10 feet. A shimmerthief constantly absorbs ambient light. Bright light in their aura becomes dim light, and dim light becomes darkness. A shimmerthief can suppress or activate this aura as a single action with the [[Traits/Concentrate (Player Core)|concentrate]] trait.

---

**Speed** 25 feet

**Melee**
single action
jaws + 17,
**Damage** 2d8+10 piercing

**Melee**
single action
claw +17 ([[Traits/Agile]]),
**Damage** 2d6+10 slashing

**Primal Innate Spells** DC 25, attack +17
- **2nd**
[[Spells/Blur]], [[Spells/Darkness]] (at will)
- **4th**
[[Spells/Darkness]]

**Drain Light** two actions ([[Traits/Abjuration]], [[Traits/Darkness]], [[Traits/Primal]]) The shimmerthief targets a light source within 30 feet, then attempts to counteract it with a spell attack roll. If they succeed, the shimmerthief regains 2d8+10 Hit Points (4d8+20 Hit Points on a critical success).

**Lasting Darkness** ⤾  **Frequency** once per day; **Trigger** The shimmerthief casts darkness as a 2nd-level spell; **Effect** The duration of the shimmerthief's darkness increases to 24 hours.

**Critical Success** The creature is unaffected.  

**Success** The creature is [[Rules/Conditions/Frightened (Player Core)|frightened]] 1.  

**Failure** The creature is frightened 2.  

**Critical Failure** The creature is frightened 2 and [[Rules/Conditions/Fleeing (Player Core)|fleeing]] for as long as it remains frightened.

> [!info] Sidebar
> ## Alternate Diets
>
>
>
> Some shimmerthieves develop a taste for and the capacity to consume energy forms far more insidious than light, allowing them to feed on life energy or thought energy instead of light. Such behavior is considered monstrous by most shimmerthieves and a reason for exile. Shimmerthieves with these proclivities are dangerous predators that maintain the ability to feed on light and purposely create pockets of darkness to trap and feed on victims. A shimmerthief that feeds on life energy deals negative damage with their jaws, while one that feeds on thought energy deals mental damage with their jaws.

---

Source on Archives of Nethys: [Shimmerthief](https://2e.aonprd.com/Monsters.aspx?ID=2795)
