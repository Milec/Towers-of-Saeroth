---
title: "Oread Guard"
type: "creature"
aon_id: "creature-760"
aon_url: "https://2e.aonprd.com/Monsters.aspx?ID=760"
tags:
  - pf2e/creature
  - rarity/common
  - trait/human
  - trait/humanoid
  - trait/oread
  - trait/ln
  - trait/medium
ac: 19
alignment: "LN"
attack_bonus:
  - 6
  - 9
charisma: 0
constitution: 2
creature_ability:
  - "Attack of Opportunity"
  - "Shield Block"
  - "Power Attack"
creature_family: "Geniekin"
dexterity: 1
fortitude_save: 7
hp: 20
intelligence: 0
is_standard_ancestry_feat: false
item:
  - "Bastard Sword"
  - "Full Plate"
  - "light hammer"
  - "Steel Shield (Hardness 5, HP 20, BT 10)"
language:
  - "Common"
  - "Terran"
level: 1
npc: false
perception: 7
pfs: "Standard"
primary_source: "Bestiary 2"
primary_source_category: "Rulebooks"
rarity: "common"
reflex_save: 6
release_date: "2020-05-27"
size:
  - "Medium"
skill:
  - "Athletics"
  - "Crafting"
  - "Medicine"
  - "Society"
  - "Survival"
skill_mod:
  - "athletics 7"
  - "crafting 3"
  - "medicine 5"
  - "society 3"
  - "survival 5"
source:
  - "Bestiary 2"
source_category:
  - "Rulebooks"
speed:
  - "land 20"
strength: 4
strike_damage_average:
  - 7
  - 8
strongest_save:
  - "fort"
  - "fortitude"
trait:
  - "Human"
  - "Humanoid"
  - "Oread"
  - "LN"
  - "Medium"
trait_group:
  - "Ancestry"
  - "Half-Elf"
  - "Half-Orc"
  - "Creature Type"
  - "Monster"
weakest_save:
  - "will"
will_save: 5
wisdom: 2
---

# Oread

Elemental earth laces through the bones of oreads, who appear similar to stone statues of their mortal ancestry, with delicate crystals in place of hair, fur, or scales. Oreads are stoic and slow to plan but are of steadfast resolve and unwavering in their convictions.

 The typical oread cherishes quiet seclusion. Yet as they age, many oreads find themselves inexplicably drawn to some far-flung location with a pull like that exerted on the needle of a compass—intangible, constant, and ultimately irresistible. The destination of this mysterious pilgrimage is unique to each oread, though it usually ends in some place of great mystical power, natural splendor, or esoteric learning. Most oreads are drawn to a place with which they are somewhat familiar, but a rare few feel drawn to travel in a seemingly random direction, departing with only their hope that they'll discover whatever mystery lies at the end of their invisible path.

 Many oreads find that the role of a guard suits their personality well, for in such a role they can feel as if they are helping to promote order but also find time to stand vigil as lone sentinels over a specific portion of a fortification or a remote location on a wilderness trail.

**[[Rules/Recall Knowledge|Recall Knowledge - Humanoid]]**
([[Rules/Sidebars/Examining Forgeries|Society]]): DC 15

**[[Rules/Recall Knowledge|Unspecific Lore]]**: DC 13

**[[Rules/Recall Knowledge|Specific Lore]]**: DC 10

## Oread Guard  *Creature 1*

**Traits** [[Rules/Alignment (Core Rulebook)|LN]], Medium, [[Traits/Human]], [[Traits/Humanoid]], [[Traits/Oread]]

**Source** [[Sources/Bestiary 2]] pg. 201

**Perception** +7

**Languages**
[[Setting/Languages/Common]], [[Setting/Languages/Terran]]

**Skills**
[[Rules/Sidebars/Falling|Athletics]] +7, [[Rules/Sidebars/Crafting Example|Crafting]] +3, [[Rules/Skills/Medicine]] +5, [[Rules/Sidebars/Examining Forgeries|Society]] +3, [[Rules/Skills/Survival]] +5

**Str** +4

**Dex** +1

**Con** +2

**Int** +0

**Wis** +2

**Cha** +0

**Items**
[[Equipment/Weapons/Bastard Sword]], [[Equipment/Armor/Full Plate]], [[Equipment/Weapons/Light Hammer|light hammer]], [[Equipment/Shields/Steel Shield]] (Hardness 5, HP 20, BT 10)

---

**AC** 19 (21 with shield raised)

**Fort** +7

**Ref** +6

**Will** +5

**HP** 20

**[[Bestiary/Abilities/Attack of Opportunity]]** ⤾  
**[[Bestiary/Abilities/Shield Block]]** ⤾

---

**Speed** 20 feet

**Melee**
single action
bastard sword +9 ([[Traits/Two-Hand|two-hand d12]]),
**Damage** 1d8+4 slashing

**Ranged**
single action
light hammer +6 ([[Traits/Agile]], [[Traits/Thrown|thrown 20 feet]]),
**Damage** 1d6+4 bludgeoning

**Power Attack** two actions  **Frequency** once per round; **Effect** The guard makes a melee Strike. This counts as two attacks when calculating the guard's multiple attack penalty. If this Strike hits, the guard deals an extra die of weapon damage.

---

Source on Archives of Nethys: [Oread Guard](https://2e.aonprd.com/Monsters.aspx?ID=760)
