---
title: "Cyclops"
type: "creature"
aon_id: "creature-86"
aon_url: "https://2e.aonprd.com/Monsters.aspx?ID=86"
tags:
  - pf2e/creature
  - rarity/uncommon
  - trait/giant
  - trait/humanoid
  - trait/uncommon
  - trait/le
  - trait/large
ac: 21
alignment: "LE"
attack_bonus:
  - 8
  - 14
charisma: -1
constitution: 2
creature_ability:
  - "Ferocity"
  - "Flash of Insight"
  - "Swipe"
creature_family: "Cyclops"
dexterity: -1
fortitude_save: 13
hp: 80
intelligence: 0
is_standard_ancestry_feat: false
item:
  - "Hide Armor"
  - "Heavy Crossbow (10 bolts)"
language:
  - "Common"
  - "Cyclops"
  - "Jotun"
level: 5
npc: false
perception: 12
pfs: "Standard"
primary_source: "Bestiary"
primary_source_category: "Rulebooks"
rarity: "uncommon"
reflex_save: 8
release_date: "2019-08-01"
sense: "low-light vision"
size:
  - "Large"
skill:
  - "Athletics"
  - "Fortune-Telling Lore"
  - "Intimidation"
  - "Survival"
skill_mod:
  - "athletics 14"
  - "intimidation 10"
  - "survival 12"
source:
  - "Bestiary"
source_category:
  - "Rulebooks"
speed:
  - "land 30"
strength: 5
strike_damage_average:
  - 9
  - 15
strongest_save:
  - "fort"
  - "fortitude"
trait:
  - "Giant"
  - "Humanoid"
  - "Uncommon"
  - "LE"
  - "Large"
trait_group:
  - "Creature Type"
  - "Rarity"
vision: "Low-light vision"
weakest_save:
  - "ref"
  - "reflex"
will_save: 12
wisdom: 3
---

# Cyclops

The kingdoms of the cyclopes date to an age before the rise of humanity, when dragons and giants and serpentfolk ruled the world. The cyclopes built enormous stone cities and prayed to ancient gods of brutality and wrath, but their power to foresee the future failed them, and their civilization collapsed. Today, most cyclopes have virtually no knowledge of the former glory of their kind, even though it is not uncommon for them to dwell among the ruins of their greatness. Cyclops cities include monuments and imposing murals which depict their peoples’ history, but few now among them can read or interpret these relics of the past.

 In addition to their single eye, cyclopes are also famous for their never-ending hunger, an appetite so all-consuming that some scholars theorize it may in fact be some kind of curse. The ever-present hunger of the cyclopes seems to have some connection to the death of their civilization—though whether this voracity was the cause or a side-effect of their people’s downfall is likely destined to remain a mystery.

 Although details of the cyclopes’ gods have largely been lost to the annals of time, what little is known about these deities suggests they were vindictive and petty enough to curse their own people if they felt neglected or badly served.

**[[Rules/Recall Knowledge|Recall Knowledge - Humanoid]]**
([[Rules/Sidebars/Examining Forgeries|Society]]): DC 22

**[[Rules/Recall Knowledge|Unspecific Lore]]**: DC 20

**[[Rules/Recall Knowledge|Specific Lore]]**: DC 17

## Cyclops  *Creature 5*

**Traits** [[Traits/Uncommon]], [[Rules/Alignment (Core Rulebook)|LE]], Large, [[Traits/Giant]], [[Traits/Humanoid]]

**Source** [[Sources/Bestiary]] pg. 68

**Perception** +12; low-light vision

**Languages**
[[Setting/Languages/Common]], [[Setting/Languages/Cyclops]], [[Setting/Languages/Jotun]]

**Skills**
[[Rules/Sidebars/Falling|Athletics]] +14, [[Rules/Sidebars/Common Lore Subcategories|Fortune-Telling Lore]] +13, [[Rules/Skills/Intimidation]] +10, [[Rules/Skills/Survival]] +12

**Str** +5

**Dex** -1

**Con** +2

**Int** +0

**Wis** +3

**Cha** -1

**Items**
[[Equipment/Armor/Hide Armor]], [[Equipment/Weapons/Heavy Crossbow]] (10 bolts)

---

**AC** 21

**Fort** +13

**Ref** +8

**Will** +12

**HP** 80

**[[Bestiary/Abilities/Ferocity]]** ⤾  
**Flash of Insight** free action ([[Traits/Divination|divination]], [[Traits/Occult|occult]], [[Traits/Fortune|fortune]]) **Frequency** once per day; **Trigger** The cyclops is about to roll a d20. **Effect** The cyclops peers into an occluded spectrum of possible futures. It gets a success (but not a critical success) on the roll instead of rolling.

---

**Speed** 30 feet

**Melee**
single action
greataxe +14 ([[Traits/Reach|reach 10 feet]], [[Traits/Sweep]]),
**Damage** 1d12+9 slashing

**Ranged**
single action
heavy crossbow +8 ([[Traits/Range|range increment 120 feet]], [[Traits/Reload|reload 2]]),
**Damage** 1d10+4 piercing

**Swipe** two actions ([[Traits/Flourish]]) The cyclops makes a melee Strike and compares the attack roll result to the AC of up to two foes, each of whom must be within its melee reach and adjacent to each other. Roll damage only once and apply it to each creature hit. A Swipe counts as two attacks for the cyclops’s multiple attack penalty.

---

Source on Archives of Nethys: [Cyclops](https://2e.aonprd.com/Monsters.aspx?ID=86)
