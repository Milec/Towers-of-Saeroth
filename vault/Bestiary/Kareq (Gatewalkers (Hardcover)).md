---
title: "Kareq"
type: "creature"
aon_id: "creature-4231"
aon_url: "https://2e.aonprd.com/Monsters.aspx?ID=4231"
tags:
  - pf2e/creature
  - rarity/uncommon
  - trait/animal
  - trait/uncommon
  - trait/large
ac: 22
attack_bonus:
  - 14
charisma: 0
constitution: 4
creature_ability:
  - "Boomstrike"
dexterity: 2
fortitude_save: 15
hp: 95
immunity:
  - "sonic"
intelligence: -4
is_standard_ancestry_feat: false
level: 5
npc: false
perception: 15
primary_source: "Gatewalkers (Hardcover)"
primary_source_category: "Adventure Paths"
primary_source_group: "Gatewalkers"
rarity: "uncommon"
reflex_save: 13
release_date: "2025-07-02"
sense: "low-light vision"
size:
  - "Large"
skill:
  - "Acrobatics"
  - "Athletics"
  - "Intimidation"
  - "Stealth"
  - "Survival"
skill_mod:
  - "acrobatics 12"
  - "athletics 13"
  - "intimidation 13"
  - "stealth 12"
  - "survival 10"
source:
  - "Gatewalkers (Hardcover)"
source_category:
  - "Adventure Paths"
source_group:
  - "Gatewalkers"
speed:
  - "climb 20"
  - "land 40"
spoilers: "Gatewalkers"
strength: 5
strike_damage_average:
  - 16
strongest_save:
  - "fort"
  - "fortitude"
trait:
  - "Animal"
  - "Uncommon"
  - "Large"
trait_group:
  - "Creature Type"
  - "Rarity"
vision: "Low-light vision"
weakest_save:
  - "will"
will_save: 10
wisdom: 3
---

# Kareq

A kareq looks like a cross between a dinosaur and an insect. The kareq's torso extends back into a bulbous abdomen like an ant's.

A kareq has a resonance chamber in its lower body, and when it flexes its muscular back legs against it, pressure builds up and sound waves are amplified and then focused as the kareq opens its mouth and releases the blast. The resulting shock wave can easily kill.

Kareqs are thought to be related to Golarion's [[Bestiary/Destrachan|destrachans]], though their similarities may simply be the result of convergent evolution. Unlike the stronger subterranean destrachans, kareqs are native to a variety of different environments, most often jungles and forests. A few sandy-colored species are found in the deserts of the Colonies, where formians regard the beasts as noble or, in some cases, practically sacred.

Kareqs tend to be solitary predators, bold and unafraid, even willing to take on predators significantly larger than themselves. In combat, they keep their distance at first, softening up opponents with their boomstrikes before moving in for the kill with their claws.

While they don't usually hunt in populated areas, kareqs are extremely territorial, and their desire to lay claim to tall landscape features can bring them into conflict with humanoids. During mating season, a kareq seeks height, climbing to the top of hills, boulders, or—if a settlement is particularly unlucky—towers and other buildings. From these perches, kareqs blast out their booming mating call which can carry for dozens of miles.

A typical kareq stands 5 feet tall at the shoulder and is 10 feet long, weighing up to 300 pounds.

**[[Rules/Recall Knowledge|Recall Knowledge - Animal]]**
([[Rules/Skills/Nature (Player Core)|Nature]]): DC 22

**[[Rules/Recall Knowledge|Unspecific Lore]]**: DC 20

**[[Rules/Recall Knowledge|Specific Lore]]**: DC 17

## Kareq  *Creature 5*

**Traits** [[Traits/Uncommon (Player Core)|Uncommon]], Large, [[Traits/Animal (Player Core)|Animal]]

**Source** [[Sources/Gatewalkers (Hardcover)]] pg. 263

**Perception** +15; low-light vision

**Languages**

**Skills**
[[Rules/Skills/Acrobatics (Player Core)|Acrobatics]] +12, [[Rules/Sidebars/Multiple Attacks with Athletics (Player Core)|Athletics]] +13, [[Rules/Skills/Intimidation (Player Core)|Intimidation]] +13, [[Rules/Sidebars/Being Stealthy (Player Core)|Stealth]] +12, [[Rules/Skills/Survival (Player Core)|Survival]] +10

**Str** +5

**Dex** +2

**Con** +4

**Int** -4

**Wis** +3

**Cha** +0

---

**AC** 22

**Fort** +15

**Ref** +13

**Will** +10

**HP** 95

**Immunities**
[[Traits/Sonic (Player Core)|sonic]]

---

**Speed** 40 feet, climb 20 feet

**Melee**
single action
claw +14,
**Damage** 2d10+5 slashing

**Boomstrike** two actions  The kareq beats its legs against its reverberation chamber, creating a blast of sound concentrated enough to ignite the air around it. The kareq chooses to affect either a 30- foot [[Rules/Cone (Player Core)|cone]] (which deals sonic damage) or a 60-foot [[Rules/Line (Player Core)|line]] (which deals fire damage). Affected creatures must attempt a DC 21 Fortitude save.  
**Success** The creature takes no damage.  
**Failure** The creature takes 3d6 damage of the appropriate type.  
**Critical Failure** The creature takes 6d6 damage of the appropriate type and either is [[Rules/Conditions/Deafened (Player Core)|deafened]] for 1 minute (if sonic damage) or takes 1d6 [[Rules/Sidebars/Persistent Damage Rules (Player Core)|persistent fire damage]] (if fire damage).

> [!info] Sidebar
> ## Kareq Drums
>
>
>
> Castrovelians have long hunted kareqs, both for glory and to harvest their unique abdomens. A ceremonial drum made from a kareq resonation chamber is an intimidating mark of prestige. While most such drums are only instruments, a properly enchanted kareq war drum can bring down walls and blast apart enemy troops.

---

Source on Archives of Nethys: [Kareq](https://2e.aonprd.com/Monsters.aspx?ID=4231)
