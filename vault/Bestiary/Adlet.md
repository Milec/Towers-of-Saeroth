---
title: "Adlet"
type: "creature"
aon_id: "creature-1054"
aon_url: "https://2e.aonprd.com/Monsters.aspx?ID=1054"
tags:
  - pf2e/creature
  - rarity/common
  - trait/cold
  - trait/humanoid
  - trait/cn
  - trait/medium
ac: 30
alignment: "CN"
attack_bonus:
  - 19
  - 20
  - 21
charisma: 0
constitution: 4
creature_ability:
  - "Wolfstorm"
  - "Avenging Bite"
  - "Frozen Weapons"
  - "Pack Attack"
  - "Wolfrime"
dexterity: 6
fortitude_save: 20
hp: 180
immunity:
  - "cold"
intelligence: 0
is_standard_ancestry_feat: false
item:
  - "+1 striking spear (2)"
language:
  - "Adlet"
  - "Common"
level: 10
npc: false
perception: 18
pfs: "Standard"
primary_source: "Bestiary 3"
primary_source_category: "Rulebooks"
rarity: "common"
reflex_save: 22
release_date: "2021-04-07"
sense: "low-light vision , scent (imprecise) 30 feet"
size:
  - "Medium"
skill:
  - "Acrobatics"
  - "Athletics"
  - "Stealth"
  - "Survival"
skill_mod:
  - "acrobatics 20"
  - "athletics 21"
  - "stealth 20"
  - "survival 18"
source:
  - "Bestiary 3"
source_category:
  - "Rulebooks"
speed:
  - "land 40"
stealth: "20"
strength: 5
strike_damage_average:
  - 19
  - 19
  - 21
strongest_save:
  - "ref"
  - "reflex"
trait:
  - "Cold"
  - "Humanoid"
  - "CN"
  - "Medium"
trait_group:
  - "Energy"
  - "Monster"
  - "Creature Type"
vision: "Low-light vision"
weakest_save:
  - "will"
weakness:
  - "fire 10"
will_save: 16
wisdom: 4
---

# Adlet

Adlets dwell in the farthest, harshest reaches of the Crown of the World. At first glance, these isolated people look much like their Erutaki human cousins; they typically have terra-cotta skin, straight black hair, and compact, powerful builds. However, adlets tend to be taller and more sinewy than their human kin. Up close, adlets' strangeness reveals itself—each sports a maw full of wolflike teeth. Their legs and tail resemble that of a dog.

 Adlets' legends say that long ago, a mighty hunter lost his way far from home and came upon a house of whalebone and ice. A woman dressed in white fox furs greeted him, fed him, and tended to his frostbite. In time, they married and had 10 children, five of whom bore the legs and tails of foxes. These children stayed with their mother, while the other five—born with the legs and tails of wolves— traveled with their father back to the human lands and became the first adlets.

 Adlets aren't inherently evil, but their culture is warlike, xenophobic, and noticeably lacking in humility. They see themselves as the natural rulers of the arctic wastes and view everyone else as squatters at best and invaders at worst. A typical adlet is stronger and faster than any mundane human, with the ability to walk naked in a blizzard and call up ice-cold mists. Given that, it's little wonder that adlets have developed something of a superiority complex. Still, while adlet raids are a common problem for travelers in the Crown of the World, a handful of wily and intrepid merchants have forged peaceful relations with certain adlet communities along more common routes.

**[[Rules/Recall Knowledge|Recall Knowledge - Humanoid]]**
([[Rules/Sidebars/Examining Forgeries|Society]]): DC 27

**[[Rules/Recall Knowledge|Unspecific Lore]]**: DC 25

**[[Rules/Recall Knowledge|Specific Lore]]**: DC 22

## Adlet  *Creature 10*

**Traits** [[Rules/Alignment (Core Rulebook)|CN]], Medium, [[Traits/Cold]], [[Traits/Humanoid]]

**Source** [[Sources/Bestiary 3]] pg. 9

**Perception** +18; [[Bestiary/Abilities/Low-Light Vision|low-light vision]], [[Bestiary/Abilities/Scent|scent]] (imprecise) 30 feet

**Languages**
[[Setting/Languages/Adlet]], [[Setting/Languages/Common]]

**Skills**
[[Rules/Skills/Acrobatics]] +20, [[Rules/Sidebars/Falling|Athletics]] +21, [[Rules/Sidebars/Being Stealthy|Stealth]] +20, [[Rules/Skills/Survival]] +18

**Str** +5

**Dex** +6

**Con** +4

**Int** +0

**Wis** +4

**Cha** +0

**Items**
_+1 [[Equipment/Striking|striking]] [[Equipment/Weapons/Spear|spear]]_ (2)

---

**AC** 30

**Fort** +20

**Ref** +22

**Will** +16

**HP** 180

**Immunities**
cold

**Weaknesses**
fire 10

**Wolfstorm** ([[Traits/Aura|aura]], [[Traits/Cold|cold]], [[Traits/Evocation|evocation]], [[Traits/Primal|primal]]) 60 feet. A clammy, frigid mist billows forth ahead of the adlet. Creatures within the mist become [[Rules/Conditions/Concealed|concealed]], and creatures outside the mist become concealed to creatures within it. An adlet can see through the aura without penalty.  
**Avenging Bite** ⤾ **Trigger** A creature within reach of an adlet's jaws Strike attacks one of the adlet's allies; **Effect** The adlet makes a jaws Strike against the triggering creature.

---

**Speed** 40 feet

**Melee**
single action
_spear_ +20 ([[Traits/Magical]]),
**Damage** 2d6+9 piercing plus 1d6 cold

**Melee**
single action
jaws +19,
**Damage** 2d8+9 slashing plus 1d6 cold

**Ranged**
single action
spear +21 ([[Traits/Magical]], [[Traits/Thrown|thrown 20 feet]]),
**Damage** 2d6+9 piercing plus 1d6 cold

**Frozen Weapons**  ([[Traits/Evocation]]) Weapons wielded by an adlet gain the effect of the [[Equipment/Frost|_frost_]] property rune.

**Pack Attack**   An adlet's Strikes deal an additional 2d6 damage to creatures that are within the reach of at least two of the adlet's allies.

**Wolfrime** single action ([[Traits/Cold]], [[Traits/Concentrate]], [[Traits/Evocation]], [[Traits/Primal]]) An adlet's mist turns biting cold and coalesces into a thick rime of frost that deals 6d6 cold damage to creatures inside the adlet's wolfstorm aura (DC 26 basic Fortitude), and the aura is deactivated until the start of the adlet's next turn.

> [!info] Sidebar
> ## Lost Cousins
>
>
>
> Legends about adlets' origins suggest they might have fox-like kin. Some believe these kin are [[Rules/Sidebars/Fox Families|kitsune]], while others point to the more powerful [[Bestiary/Vulpinal|vulpinals]] as more likely progenitors.

---

Source on Archives of Nethys: [Adlet](https://2e.aonprd.com/Monsters.aspx?ID=1054)
