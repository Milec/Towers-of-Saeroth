...
const travelView=document.createElement('details');travelView.id='travelView';travelView.className='atlas-controls travel-view';travelView.open=true;
travelView.innerHTML=`<summary>Travel view</summary><label><input id="travelRouteFocus" type="checkbox"> Emphasize routes</label><p class="muted">Keeps roads, trails, and sea lanes easy to follow. Selecting a route or planning a journey turns this on automatically.</p><p id="partyLocation" class="party-location" role="status" aria-live="polite">Party location: not marked.</p><button type="button" id="clearPartyLocation" hidden>Clear party marker</button>`;
journey.after(travelView);
...
const partyGroup=document.createElementNS(NS,'g');partyGroup.id='partyLocationOverlay';map.insertBefore(partyGroup,$('#labels'));
const travelFocus=$('#travelRouteFocus');
const setTravelFocus=on=>{travelFocus.checked=on;map.classList.toggle('travel-focus',on);try{localStorage.setItem('saeroth-travel-focus',String(on));}catch{}};
window.setAtlasTravelFocus=setTravelFocus;
try{setTravelFocus(localStorage.getItem('saeroth-travel-focus')==='true');}catch{setTravelFocus(false);}
travelFocus.onchange=()=>setTravelFocus(travelFocus.checked);
let partyPlace=null;
const renderPartyMarker=(unit=box[2]/Math.max(map.clientWidth,300))=>{const p=partyPlace&&places.find(place=>place.key===partyPlace.key);partyPlace=p||null;partyGroup.innerHTML='';const status=$('#partyLocation'),clear=$('#clearPartyLocation');if(!p){status.textContent='Party location: not marked.';clear.hidden=true;return;}const node=window.ATLAS_NETWORK?.nodes?.[p.cell];if(!node){status.textContent='Party location: '+p.name+' (no atlas position available).';clear.hidden=false;return;}partyGroup.innerHTML=`<g class="party-marker" transform="translate(${node[0]} ${node[1]}) scale(${unit})"><circle class="party-marker-halo" r="20"/><circle class="party-marker-pin" r="9"/><path class="party-marker-arrow" d="M0,-6L5,4H-5Z"/><text y="-28" text-anchor="middle">Party</text></g>`;status.textContent='Party location: '+p.name+'.';clear.hidden=false;};
window.renderPartyMarker=renderPartyMarker;
const setPartyLocation=p=>{partyPlace=p?{key:p.key,name:p.name,cell:p.cell}:null;try{if(partyPlace)localStorage.setItem('saeroth-party-location',JSON.stringify(partyPlace));else localStorage.removeItem('saeroth-party-location');}catch{}renderPartyMarker();if(p){$('#journeyFrom').value=p.label;setTravelFocus(true);}};
try{const saved=JSON.parse(localStorage.getItem('saeroth-party-location')||'null');if(saved?.key)partyPlace=saved;}catch{}renderPartyMarker();
$('#clearPartyLocation').onclick=()=>setPartyLocation(null);
