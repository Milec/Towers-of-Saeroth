...
renderSettlementNames=function(z,unit){
 ...
 const selectionLayer=selected?.type==='custompoi'?'custompois':selected?.type==='burg'?'settlements':selected?.type==='poi'?'pois':selected?.type==='crossing'?'crossings':selected?.type==='nation'?'countries':selected?.type==='province'?'provinces':selected?.type==='subprovince'?'subprovinces':selected?.type==='route'?D.routes.find(r=>r.i===selected.id)?.group:null;
