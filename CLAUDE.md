...
**`campaign/Saeroth.map` is not generated by `build.js`.** It is a map the vault
kept, made on Azgaar 1.143.2 and brought forward to 1.151.1 by
`tools/mapgen/modernize.js`, which loads a finished map and adds only what the
generator has grown since it was saved: transports and the nine trade journeys,
provinces, armies, statistics, and the diplomacy re-applied from the note. It
touches no terrain, no territory, no burgs and no names, so **rerun it rather
than `build.js` after editing `Political Relations.md` or `Trade Routes.md`** —
`build.js` would forge a different world. `inspect.py` does not score this map;
it reads a profile the forge writes while working, and a map that arrived as a
file has none. `verify.js` does apply.

`tools/mapgen/journeys.js` holds the corridor code both scripts share, because a
...
all 16 required borders survive the split, and it fixes the climate-band
crowding — and it was set aside: the three maps come out at three different
scales with no merge tool to put them back together.
