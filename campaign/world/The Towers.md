...
Very little, and the little is all from the outside:
...
- Expeditions are financed the way any venture is financed, which means the
  [[Thurion Merchant Alliance]] underwrites a great many of them and has begun
  quietly refusing to insure a second attempt on the same tower.
- Every succession crisis on the board now has a tower-shaped answer to it. An
  heir with no claim, a khan with no votes, a house short of a majority: a
  conquered tower would settle any of those arguments permanently, which is why
...
has been forthcoming about what its own people found. If a tower can be entered
from below, then a structure "appearing" is only what it looks like from the
surface — and the question changes from *what arrived* to *what finally reached
us*.
